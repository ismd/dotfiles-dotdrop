...
(use-package copilot-chat
  ...
  ;; Recommendation: For generating Git commit messages:
  ;; - **If you want high-quality, context-aware messages**: Go with **GPT-4.1** or **Gemini-2.0-flash-001**.
  ;; - **If you prioritize speed and efficiency**: Use **GPT-4o**.
  ;; - **If you're working in a resource-constrained environment**: Opt for **O3-mini**.
  (copilot-chat-commit-model "claude-haiku-4.5")
...
I don't use this currently, as I prefer to use =perspective= which has it's own session management.

#+begin_src emacs-lisp
;; Delay desktop restoration until after Elpaca finishes loading packages
;; This ensures all hooks are registered before buffers are restored
...
(use-package general
  ...
  :demand t
  :config
  (defun ismd/buffer-switcher (arg)
    "Show buffer switcher using perspective-aware bs-show if available.
With prefix ARG, pass it to the buffer switcher function."
    (interactive "P")
    (require 'bs)
  ...
  (general-define-key
   ...
   "C-s" '(swiper :wk "Search for string")
  ...
  (ismd/leader-keys-C-x
    ...
    "b" '(ismd/buffer-switcher :wk "Buffer switcher")
...
I use =git-gutter-fringe= because as I understand =git-gutter= doesn't work with Doom Themes.
...
;; Disabled because misaligned icons in treemacs
