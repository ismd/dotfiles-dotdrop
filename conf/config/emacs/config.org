...
(use-package general
  ...
  (cz/leader-keys
    ...
    "a c a" '(copilot-chat-add-current-buffer :wk "Add current buffer")
    "a c c" '(copilot-chat-display :wk "Chat display")
    "a c d" '(copilot-chat-del-current-buffer :wk "Delete current buffer")
    "a c l" '(copilot-chat-list :wk "Copilot chat list")
    "a c M-RET" '(copilot-chat-transient :wk "Copilot chat transient")
    ;; TODO copilot-chat-transient-code
