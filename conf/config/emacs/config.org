...
- [[#archived-configurations][ARCHIVED CONFIGURATIONS]]
  - [[#ai-tools-archived][AI Tools (Archived)]]
  - [[#completion-frameworks-archived][Completion Frameworks (Archived)]]
  - [[#lsp-mode-archived][LSP Mode (Archived)]]
  - [[#git-tools-archived][Git Tools (Archived)]]
  - [[#language-modes-archived][Language Modes (Archived)]]
  - [[#python-tools-archived][Python Tools (Archived)]]
  - [[#flutter-tools-archived][Flutter Tools (Archived)]]
  - [[#project-management-archived][Project Management (Archived)]]
  - [[#org-mode-extensions-archived][Org Mode Extensions (Archived)]]
  - [[#session-management-archived][Session Management (Archived)]]
  - [[#scrolling-archived][Scrolling (Archived)]]
  - [[#ui-enhancements-archived][UI Enhancements (Archived)]]
  - [[#tab-line-archived][Tab Line (Archived)]]
  - [[#xref-archived][Xref (Archived)]]
...
These packages override built-in Emacs packages and must be loaded early to ensure the newer versions take precedence.
...
*** Load Theme
#+begin_src emacs-lisp
(defvar theme-name 'doom-one "The name of the theme to use")
...
(setq ismd/font-family "CodeNewRoman Nerd Font Mono-14"
      ismd/font-size 140)
{%@@ else @@%}
(setq ismd/font-family "CodeNewRoman Nerd Font Mono-14"
      ismd/font-size 140)
...
(defun ismd/set-fonts ()
  ...
  (set-frame-font ismd/font-family nil t)
...
(add-to-list 'default-frame-alist `(font . ,ismd/font-family))
#+end_src
...
(use-package general
  ...
  (general-define-key
   ...
   ;; Scroll
   ;; "<next>" '(good-scroll-up-full-screen :wk "Scroll up full screen")
   ;; "<prior>" '(good-scroll-down-full-screen :wk "Scroll down full screen")
   ;; "C-v" '(good-scroll-up-full-screen :wk "Scroll up full screen")
   ;; "M-v" '(good-scroll-down-full-screen :wk "Scroll down full screen")
  ...
  (ismd/leader-keys
    ...
    "C-f C-f" '(kirigami-toggle-fold :wk "Toggle fold")
    "C-f C-a C-f" '(kirigami-close-folds :wk "Fold all")
    "C-f C-a C-u" '(kirigami-open-folds :wk "Unfold all"))
  ...
  (ismd/leader-keys
    ...
    "k e" '(:keymap kele-command-map :package kele :wk "Kele"))
...
I use =git-gutter-fringe= because it works better with Doom Themes.
...
(use-package org
  ...
  (setq org-agenda-files (list (expand-file-name "~/Notes/")))
...
(defun ismd/persp-state-load ()
  "Load perspective state from default file."
...
This section contains previously used configurations that have been replaced or are no longer needed.
...
;; Supporting functions for tab-line (preserved for reference)
;; See original configuration for full implementation
#+end_src
